<template>
  <div class="w-full p-4 flex flex-col">
    ...
    <h2 class="mt-4">Copy to clipboard</h2>
    ...
    <div v-if="copyResponse" class="border mt-4 mb-8 rounded">
      <div class="bg-green w-full flex flex-row font-mono border border-b">
        <div class="p-2 w-1/4">Copy result</div>
        <div class="p-2 w-3/4 bg-grey-lightest break-words">
          {{ copyResponse | responseToString }}
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  import DeeplinkClient, { storeResponse } from './DeeplinkClient'
  ...
  export default {
    ...
    methods: {
      async initSdk () {
        ...
        this.client = await DeeplinkClient.compose(Universal, {
      ...
      copy() {
        try {
          this.copyResponse = { result: copyToClipboard(this.copyText) };
        } catch(error) {
